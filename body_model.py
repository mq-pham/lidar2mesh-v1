...
class BodyModel(nn.Module):
    ...
    def __init__(self,
        ...
        if isinstance(shapedirs, ch.Ch):
            shapedirs= shapedirs.r 
        ...
        J_regressor = smpl_dict.get('J_regressor', None)
        if sp.issparse(J_regressor):
            # Convert sparse matrix to dense and then to tensor
            J_regressor = J_regressor.toarray()
         
        
        self.comp_register('J_regressor', torch.tensor(J_regressor, dtype=dtype), persistent=persistant_buffer)
